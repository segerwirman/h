...
def close_app(target: str = ""):
    """DIAGNOSIS_2 MASALAH 3 — dulu ini Alt+F4 BUTA ke jendela fokus.

    Jendela fokus sering kali Jarvis sendiri, sehingga "tutup aplikasi
    instagram" menutup Jarvis. Sekarang target WAJIB disebut, dan
    penutupannya lewat ``actions/close_app.py`` yang berpenjaga.
    """
    name = str(target or "").strip()
    if not name:
        return ("Aplikasi mana yang harus ditutup? Sebutkan namanya — saya "
                "tidak menutup jendela yang sedang aktif begitu saja.")
    from actions.close_app import close_app as _close_named
    return _close_named(name).message


def close_window(target: str = ""):
    """Idem: tanpa target, JANGAN pakai jendela aktif sebagai default."""
    name = str(target or "").strip()
    if not name:
        return ("Jendela mana? Sebutkan judul atau nama aplikasinya — "
                "menutup jendela aktif secara buta bisa menutup saya sendiri.")
    from actions.close_app import close_app as _close_named
    return _close_named(name).message
...
def _detect_action(description: str) -> dict:
    ...
    # DIAGNOSIS_2 MASALAH 3 — aksi destruktif TIDAK ditawarkan ke penebak.
    # Prompt di bawah menyuruh model "pick the closest action"; menyertakan
    # close_app di daftar itu berarti "tutup aplikasi instagram" bisa
    # berakhir sebagai Alt+F4 ke jendela Jarvis. Penutupan bernama punya
    # jalurnya sendiri (actions/close_app.py) yang berpenjaga.
    _UNGUESSABLE = {"close_app", "close_window", "shutdown", "restart"}
    available = ", ".join(sorted(set(ACTION_MAP) - _UNGUESSABLE)) + \
...
def computer_settings(
    ...
    try:
        # Aksi penutup WAJIB membawa target. Sisanya tetap nullary seperti
        # sebelumnya — kontrak ~60 aksi lain tidak berubah.
        if action in ("close_app", "close_window"):
            named = str(value or params.get("target")
                        or params.get("app_name") or "").strip()
            return func(named)
        func()
