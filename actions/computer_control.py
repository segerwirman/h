...
def _find_model() -> str:
    """Vision model for screen_find. `gemini-2.5-flash-lite` was retired (404
    NOT_FOUND for new users); default to a current model, overridable via the
    optional `vision_model` key in config/api_keys.json."""
    try:
        import json
        from pathlib import Path
        p = Path(__file__).resolve().parent.parent / "config" / "api_keys.json"
        m = json.loads(p.read_text(encoding="utf-8")).get("vision_model")
        return m or "gemini-3.5-flash"
    except Exception:
