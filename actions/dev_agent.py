...
def _timer(log, label: str):
    return _phase_timer(log, label)
...
_DEP_STATUS_CACHE: dict[str, bool] = {}
_VSCODE_CMD_CACHE: str | None = None
...
def _get_model(model_name: str):
    ...
    wrapper = _W()
    _MODEL_CACHE[model_name] = wrapper
    return wrapper
...
def _install_dependencies(dependencies: list[str], project_dir: Path) -> str:
    ...
    for dep in dependencies:
        ...
        cached = _DEP_STATUS_CACHE.get(pkg_name)
        if cached is True:
        ...
        _DEP_STATUS_CACHE[pkg_name] = installed
        if not installed:
            to_install.append(dep)
        else:
            print(f"[DevAgent] ✓ Already installed: {pkg_name}")
...
def _try_auto_install(error_output: str, project_dir: Path) -> bool:
    ...
    if _DEP_STATUS_CACHE.get(pkg) is False:
        print(f"[DevAgent] 🔧 Skipping cached failed package: {pkg}")
        return False
    ...
    try:
        ...
        _DEP_STATUS_CACHE[pkg] = result.returncode == 0
        return result.returncode == 0
    except Exception:
        _DEP_STATUS_CACHE[pkg] = False
        return False
...
def _fix_files(
    ...
    for fp, code in file_codes.items():
        if code:
            snippet = code[:900] + ("..." if len(code) > 900 else "")
            shared_ctx.append(f"--- {fp} ---\n{snippet}")
    shared_ctx_text = "\n\n".join(shared_ctx[:4])
...
Target-specific context (read-only — fix only the target file):
{other_ctx}
...
- Prefer the smallest targeted change that resolves the error.
...
def _build_project(
        ...
        for file_info in sorted_files:
            ...
            for attempt in range(2):
                try:
                    ...
                    if attempt == 0:
                        log("Rate limit — retrying once with short backoff...")
                        time.sleep(2)
                    else:
    ...
    with _timer(log, "Install phase"):
        if dependencies:
            install_result = _install_dependencies(dependencies, project_dir)
            log(install_result)

    with _timer(log, "Open editor phase"):
        _open_vscode(project_dir)

    last_output   = ""
    auto_installs = 0  

    with _timer(log, "Run+fix phase"):
