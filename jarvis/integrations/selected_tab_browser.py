...
def _connect(port: int):
    return user_browser._connect(port)


def _release(browser) -> None:
    user_browser._release(browser)
...
class SelectedTabBrowserHost:
    ...
    def _call(self, callback, *args):
        ...
        self._jobs.put((future, callback, args))
    ...
    def _worker(self) -> None:
        while True:
            ...
            if job is _STOP:
                return
            future, callback, args = job
            if not future.set_running_or_notify_cancel():
                continue
            try:
                future.set_result(callback(*args))
            except BaseException as exc:  # owner must relay, never silently die
                future.set_exception(exc)

    def _begin_picker(self) -> PickerResult:
    ...
    def _cancel_picker(self, picker_id: str) -> bool:
        picker = self._picker
        if picker is None or picker.picker_id != picker_id:
            return False
        self._picker = None
        picker.candidates.clear()
        self._safe_release(picker.browser)
        return True

    def _select_candidate(self, picker_id: str, candidate_id: str) -> SelectionResult:
        if self._selected is not None:
    ...
    def _on_selected_target_closed(self, target_id: str) -> None:
        self._queue_lifecycle_retirement(
            target_id,
    ...
    def _on_browser_disconnected(self, connection_id: str) -> None:
        if threading.get_ident() != self._thread.ident:
            ...
            return
        self._retire_disconnected_connection(connection_id)

    def _retire_disconnected_connection(self, connection_id: str) -> None:
        picker = self._picker
    ...
    def _queue_lifecycle_retirement(self, target_id: str, reason: str) -> None:
        if threading.get_ident() != self._thread.ident:
            future: Future = Future()
            self._jobs.put(
                (future, self._retire_selected_lifecycle, (target_id, reason))
            )
            return
        self._retire_selected_lifecycle(target_id, reason)

    def _retire_selected_lifecycle(self, target_id: str, reason: str) -> None:
        selected = self._selected
        if selected is None or selected.target.target_id != target_id:
            return
        self._selected = None
        self._safe_release(selected.browser)
    ...
    def _stop_selected(self, target_id: str, target_generation: int) -> bool:
        selected = self._selected
        if selected is None:
            return False
        target = selected.target
    ...
    def _safe_release(self, browser: object) -> None:
        try:
            self._releaser(browser)
        except Exception as exc:
            _logger.warning(
